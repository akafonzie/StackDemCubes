...
glmReadMTL(GLMmodel* model, char* name)
{
	...
	GLuint numtextures2 = 0;
    
    dir = glmDirName(model->pathname);
    filename = (char*)malloc(sizeof(char) * (strlen(dir) + strlen(name) + 1));
	textureName = (char*)malloc(sizeof(char) * (strlen(dir) + strlen(name) + 1));
    strcpy(filename, dir);
	...
	model->numtextures = numtextures2;
		...
		case 'm':
			if(buf[1] == 'a')
			{
				if(buf[2] == 'p')
				{
					fgets(buf, sizeof(buf), file);
					sscanf(buf, "%s %s", buf, buf);
					strcpy(textureName, dir);
					strcat(textureName, buf);
					cout << "load this texture: " << textureName << endl;
					
					model->materials[nummaterials].materialTexture = 
						SOIL_load_OGL_texture(textureName, SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y);
					/*
					if (model->materials[nummaterials].materialTexture == 0)
						cout<<"failed to load " << buf << " material texture" << endl;
						*/
				}
			}
	...
	cout << "num of textures: " << model->numtextures << endl;
}
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group) {
		...
		if (mode & GLM_TEXTURE) {
			material = &model->materials[group->material];
			glBindTexture(GL_TEXTURE_2D, material->materialTexture);
        }
